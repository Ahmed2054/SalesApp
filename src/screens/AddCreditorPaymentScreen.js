...
import { insertCreditorPayment, updateCreditorPayment } from '../storage/database';
...
export default function AddCreditorPaymentScreen({ route, navigation }) {
  const { creditorId, creditorName, maxAmount, record } = route.params;
  ...
  const handleSave = async () => {
    ...
    // Adjust max check: if we are editing, max is maxAmount + old payment amt
    ...
    if (num > limit) {
       Alert.alert(
         ...
         `This payment (${fmt(num)}) is greater than the remaining balance (${fmt(limit)}). Continue?`,
         [
           { text: 'Cancel', style: 'cancel' },
           { text: 'Yes, Overpay', onPress: proceed }
         ]
       );
       return;
    }
  ...
  const proceed = async () => {
    ...
    try {
      ...
      console.error('[SaleApp] AddPayment error:', e);
      Alert.alert('Error', 'Could not save payment record.');
    } finally {
      setLoading(false);
    }
  };
    ...
    >
      ...
      <ScrollView contentContainerStyle={[styles.scroll, { paddingBottom: insets.bottom + 40 }]} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.section}>
          <Text style={styles.sectionTitle}>Creditor Info</Text>
          <View style={styles.card}>
             <View style={styles.infoRow}>
                <Text style={styles.infoLabel}>Paying Creditor</Text>
                <Text style={styles.infoName}>{creditorName}</Text>
             </View>
             <View style={[styles.infoRow, { borderTopWidth: 1, borderTopColor: '#f1f5f9', marginTop: 12, paddingTop: 12 }]}>
                <Text style={styles.infoLabel}>Current Debt Balance</Text>
        ...
        <View style={styles.section}>
          ...
          <View style={styles.card}>
            <View style={styles.field}>
              ...
              <TextInput
                style={[styles.input, { fontSize: 20, fontWeight: '800', color: '#b71c1c' }]}
                placeholder="0.00"
                placeholderTextColor="#b71c1c1a"
        ...
        <TouchableOpacity 
          ...
          disabled={loading}
        >
          <Text style={styles.saveBtnText}>{loading ? 'SAVING...' : 'Confirm Payment Record'}</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  infoName: { fontSize: 24, fontWeight: '900', color: '#b71c1c' },
  ...
  calendarIconBg: { width: 40, height: 40, borderRadius: 12, backgroundColor: '#fdf0f0', justifyContent: 'center', alignItems: 'center' },
  dateIcon: { fontSize: 18 },

  saveBtn: { backgroundColor: '#b71c1c', borderRadius: 18, padding: 18, alignItems: 'center', marginTop: 10, elevation: 4, shadowColor: '#b71c1c', shadowOpacity: 0.3, shadowRadius: 10 },
  ...
  iosPickerDone: { color: '#b71c1c', fontWeight: '800', fontSize: 16 },
});
