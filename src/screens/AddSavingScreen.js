...
import { insertSaving, updateSaving } from '../storage/database';
...
// ── Screen ────────────────────────────────────────────────────────────────────
export default function AddSavingScreen({ navigation, route }) {
  ...
  const handleSave = async () => {
    ...
    try {
      const record = {
        ...
        dateISO:    dateISO,
        note:       note.trim(),
        recordedAt: editing ? editing.recordedAt : new Date().toISOString(),
      };
      if (editing) {
        await updateSaving(record);
      } else {
        await insertSaving(record);
      }
      navigation.goBack();
    } catch (e) {
    ...
    >
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.card}>
            ...
            <View style={[styles.field, { borderBottomWidth: 0 }]}>
              ...
              <TouchableOpacity style={styles.datePickerBtn} onPress={openCalendar}>
                <View style={{ flex: 1 }}>
                  <Text style={styles.datePickerText}>{new Date(dateISO).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</Text>
                  <Text style={styles.datePickerDay}>{new Date(dateISO).toLocaleDateString('en-GB', { weekday: 'long' })}</Text>
                </View>
                <View style={styles.calendarIconBg}>
                  <Ionicons name="calendar" size={18} color="#00695c" />
                </View>
              </TouchableOpacity>
            </View>
          </View>
        ...
        <TouchableOpacity 
          ...
          activeOpacity={0.8}
        >
          <Text style={styles.saveBtnText}>{saving ? 'Saving...' : editing ? 'Update Saving' : 'Confirm Transaction'}</Text>
        </TouchableOpacity>
...
const styles = StyleSheet.create({
  ...
  amountInput: { flex: 1, fontSize: 24, fontWeight: '800', color: '#00695c', padding: 0 },
  ...
  calendarIconBg: { width: 40, height: 40, borderRadius: 12, backgroundColor: '#e0f2f1', justifyContent: 'center', alignItems: 'center' },
  ...
  iosPickerDone: { color: '#00695c', fontWeight: '800', fontSize: 16 },

  saveBtn: { backgroundColor: '#00695c', borderRadius: 18, padding: 18, alignItems: 'center', marginTop: 10, elevation: 4, shadowColor: '#00695c', shadowOpacity: 0.3, shadowRadius: 10 },
